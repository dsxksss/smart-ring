...
use crate::protocol::{
    imu_stream_start_packet, imu_stream_stop_packet, AccelerometerSample, ImuStreamEvaluation,
    ImuStreamSample, ImuStreamTracker,
};
...
const MIN_GRAVITY_NORM: f64 = 100.0;
const MAX_GRAVITY_NORM: f64 = 4_000.0;
...
pub async fn run(connection: RingConnection, options: ImuStreamOptions) -> Result<()> {
    ...
    tracker.start(0);
    tracing::info!("IMU_STREAM_ARMED 等待 10 Hz A2/10；250 ms 无数据将急停");
    if options.inject {
    ...
    loop {
        ...
        tokio::select! {
            _ = tick.tick() => {
                let now_ms = started.elapsed().as_millis() as u64;
                if let Some(reason) = tracker.check_timeout(now_ms) {
            ...
            packet = futures::StreamExt::next(&mut notifications) => {
                ...
                let now_ms = started.elapsed().as_millis() as u64;
                match tracker.ingest(&packet, now_ms) {
                    ImuStreamEvaluation::NotStreamPacket => {
                        tracing::debug!("忽略非 A2/10 通知");
                    }
    ...
    fn sample(sequence: u8, degrees: f64) -> ImuStreamSample {
        ...
        ImuStreamSample {
            ...
            acceleration: AccelerometerSample {
                x: (600.0 * radians.cos()).round() as i16,
                y: 0,
                z: (600.0 * radians.sin()).round() as i16,
            },
            fifo_level: 1,
            dropped_samples: 0,
            age_ticks: 0,
