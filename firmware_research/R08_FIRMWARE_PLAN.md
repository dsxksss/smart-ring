...
- OTA End 确实以 image id `0x2793` 进入 `0x00826F2A` 激活链，并经过 ROM `0x8B94/0x8B7A/0x8A5C/0x3ED1A`。这些地址的官方符号、同版本 bank 选择、运行时崩溃回滚和掉电恢复仍未证明。

## 当前离线实现

- IMU-only patch 占 280 字节，复用原厂 25 Hz FIFO 与 RAM 环形缓冲，提供 `A1 09 01/00` 启停和 10 Hz `A2 10` 通知；
- 固件端有 12 秒硬超时和生产者索引停滞急停；主机每 8 秒续期，并在 250 ms 无数据、序号跳变、校验错误或姿态无效时停止注入；
- patch SHA-256：`a157de7d40619ac9efa259e04473f7e7929ed2c82f9a9a77f25b2fc217a55928`；
- 整包候选 SHA-256：`700b362eda53135e2913c81c1a2d9e26c637d0ffa496508a5a92fdcc1ad65986`；
