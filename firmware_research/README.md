...
5. 已用 `analyze_rt08_boot_activation.py` 锁定 OTA End 到 `0x00826F2A`、再到 ROM `0x8B94/0x8B7A/0x8A5C/0x3ED1A` 的 7 组精确指令锚点；下载 payload 只有应用镜像而没有独立 OTA Bank Header，包内 `not_ready/not_obsolete` 均为 `1`；OTA End 参数为 `(image_id=0x2793, second_argument=0)`，验证成功后才提交候选地址。`emulate_rt08_stock_image_info.py` 还用 7 个场景确认原厂 `0x2790` OTA Header ID 从 resolver 对象 `+0x194` 取值，而 `0x2791..0x279A`（含 `0x2793`）从 `+0x60` 取值。该分流没有证明字段名称、安装后状态位转换、OTA Bank Header 更新/选择、运行时崩溃回滚或掉电恢复；
