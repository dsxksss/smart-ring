...
`firmware_research/scripts/emulate_rt08_imu_stream_patch.py` 使用 Unicorn 的 ARMv6-M Thumb 模式执行经 SHA-256 锁定的 280 字节补丁对象。原厂函数调用由可审计的契约 stub 代替，验证真实机器码分支、栈恢复、RAM 写入和通知包，而不是重新实现一份高层伪代码。
...
5. 生产者不前进时只发一个 `STALE`，随后停止 timer、FIFO 和传感器；
6. 第 120 个 100 ms tick 强制完整停止；
7. 显式 stop 清空状态且资源释放完整；
8. 已停止状态下的迟到 callback 无副作用。
